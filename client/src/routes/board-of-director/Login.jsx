...
export async function bodAction({ request }) {
	let formData = await request.formData();
	const data = Object.fromEntries(formData);

	let result = await http.post("/board-of-director/login", data).then((result) => result.data);
...
function BoDLogin() {
	...
	useEffect(() => {
		...
		if (user?.email) {
			setAccount(user);
			navigate("/board-of-director/pending-members");
		}
	}, [user]);
