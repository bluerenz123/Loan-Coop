	...
	return (
		<Box display="flex" flexDirection="column" height="100%">
			<Box display="flex" justifyContent="space-between">
				<Header title="NEW LOANS " subtitle="This is the summary of new loans." />
			</Box>
			<Box display="flex" flexDirection="column" height="100%" justifyContent="space-between">
				<Grid container columnSpacing={2.5} rowSpacing={2.5} mb="20px">
					<StatPaper title="3" subtitle="New Loans" icon={<RecentActorsIcon sx={{ color: colors.redAccent[500], fontSize: "26px" }} />} />
					<StatPaper
						title={PHPPrice.format(result.reduce((i, loan) => i.add(loan.monthly_deduction), new Decimal(0)).toFixed(2))}
						subtitle="Monthly Deductions"
						icon={<RecentActorsIcon sx={{ color: colors.redAccent[500], fontSize: "26px" }} />}
					/>
					<StatPaper
						title={PHPPrice.format(result.reduce((i, loan) => i.add(loan.balance), new Decimal(0)).toFixed(2))}
						...
						<TableBody>
							{result.map((loan) => (
								<TableRow key={loan.id}>
									...
									<TableCell align="center">{PHPPrice.format(loan.principal)}</TableCell>
									<TableCell align="center">{loan.terms}</TableCell>
									<TableCell align="center">{PHPPrice.format(loan.balance)}</TableCell>
									<TableCell align="center">{PHPPrice.format(loan.monthly_deduction)}</TableCell>
								</TableRow>
							))}
						</TableBody>
					</Table>
